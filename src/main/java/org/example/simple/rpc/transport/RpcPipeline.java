...
public final class RpcPipeline {
    private RpcPipeline() { }
    public static void install(Channel channel, RpcConfig config, SerializerRegistry serializers,
                               ByteBudget budget, SslContext ssl, String host, int port,
                               ChannelInboundHandler handler) {
        ChannelPipeline pipeline = channel.pipeline();
        if (ssl != null) {
            SslHandler tls = host == null ? ssl.newHandler(channel.alloc()) : ssl.newHandler(channel.alloc(), host, port);
            if (host != null) {
        ...
        pipeline.addLast("budget", limits)
            .addLast("idle", new IdleStateHandler(config.readIdleTimeoutMillis(),
                config.heartbeatIntervalMillis(), 0, TimeUnit.MILLISECONDS))
            .addLast("writeTimeout", new WriteTimeoutHandler(config.writeTimeoutMillis(), TimeUnit.MILLISECONDS))
            ...
            .addLast("heartbeat", new ChannelInboundHandlerAdapter() {
                private long ping;
                private long sequence;
                @Override public void channelRead(ChannelHandlerContext ctx, Object msg) {
                    RpcFrame frame = (RpcFrame) msg;
                    limits.framed(frame.body().length + 19);
                    if (frame.messageType() < 3) { ctx.fireChannelRead(msg); return; }
                    limits.consumed(19);
                    if (frame.messageType() == RpcProtocol.PING) {
                        ctx.writeAndFlush(new RpcFrame(RpcProtocol.PONG, (byte) 0, frame.requestId(), new byte[0]));
                    } else if (ping != frame.requestId()) { ctx.close(); }
                    else { ping = 0; }
                }
                @Override public void userEventTriggered(ChannelHandlerContext ctx, Object event) {
                    if (event instanceof IdleStateEvent idle) {
                        if (idle.state() == IdleState.READER_IDLE) {
                            org.example.simple.rpc.monitoring.RpcTelemetry.event("heartbeat.timeout"); ctx.close();
                        }
                        else if (idle.state() == IdleState.WRITER_IDLE && ping == 0) {
                            ping = ++sequence;
                            ctx.writeAndFlush(new RpcFrame(RpcProtocol.PING, (byte) 0, ping, new byte[0]));
                        }
    ...
    public static ChannelFuture write(Channel channel, RpcFrame frame, ByteBudget budget) {
        ...
        if (!budget.acquire(frame.body().length)) {
            promise.setFailure(new IllegalStateException("待编码报文预算已满")); channel.close(); return promise;
        }
        Runnable action = () -> {
            try {
                if (channel.isActive()) { channel.writeAndFlush(frame, promise); }
                else { promise.tryFailure(new IllegalStateException("连接已关闭")); }
            } finally { budget.release(frame.body().length); }
        };
        if (channel.eventLoop().inEventLoop()) { action.run(); }
        else {
            try { channel.eventLoop().execute(action); }
            catch (RuntimeException error) { budget.release(frame.body().length); promise.tryFailure(error); }
        }
        return promise;
    }
