...
public final class RpcExecutors {
    ...
    public static ThreadPoolExecutor pool(String role, int threads, int capacity) {
        return new ThreadPoolExecutor(threads, threads, 0, TimeUnit.MILLISECONDS,
            new ArrayBlockingQueue<>(capacity), factory(role), new ThreadPoolExecutor.AbortPolicy());
    }
    /** 为排队任务绑定取消清理，shutdownNow 返回的任务同样释放报文与准入资源。 */
    public static final class Lease implements Runnable {
        private final AtomicInteger references = new AtomicInteger(1);
        private final Runnable release;
        public Lease(Runnable release) { this.release = release; }
        public void retain() { references.incrementAndGet(); }
        public void run() { if (references.decrementAndGet() == 0) { release.run(); } }
    }
    public static final class Task implements Runnable {
        ...
        public Task(Runnable action, Runnable cleanup) { this.action = action; this.cleanup = cleanup; }
        public void run() {
            if (!claimed.compareAndSet(false, true)) { return; }
            try { action.run(); } finally { cleanup.run(); }
        }
        public void discard() { if (claimed.compareAndSet(false, true)) { cleanup.run(); } }
    }
    public static void stop(ThreadPoolExecutor executor) {
        for (Runnable task : executor.shutdownNow()) {
            if (task instanceof Task owned) { owned.discard(); }
            else { task.run(); }
        }
    }
