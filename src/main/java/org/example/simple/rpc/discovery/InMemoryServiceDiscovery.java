...
public final class InMemoryServiceDiscovery implements ServiceDiscovery {
    ...
    public synchronized void update(String service, List<ServiceInstance> instances) {
        ...
        listeners.getOrDefault(service, new CopyOnWriteArrayList<>()).forEach(listener -> listener.accept(snapshot));
    }
    @Override public synchronized AutoCloseable subscribe(String service, Consumer<List<ServiceInstance>> listener,
                                                           Consumer<Throwable> errors) {
        var group = listeners.computeIfAbsent(service, key -> new CopyOnWriteArrayList<>());
        group.add(listener); listener.accept(values.getOrDefault(service, List.of()));
