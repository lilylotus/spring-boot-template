...
/**
 * AES-GCM 密钥生成与认证加解密工具。
 * <p>
 * 密文格式为“版本字节、96 位随机 IV、密文与 128 位认证标签”，组合后使用标准 Base64 编码。
 */
public final class AesUtils {

    /** 对称密钥算法名称，用于生成 AES 密钥和构造密钥对象。 */
    private static final String ALGORITHM = "AES";

    /** AES 加解密转换名称，GCM 模式同时提供数据机密性与完整性校验且不需要填充。 */
    private static final String TRANSFORMATION = "AES/GCM/NoPadding";

    /** AES 密钥位数，固定为 256 位以提供高强度对称加密。 */
    private static final int KEY_SIZE_BITS = 256;

    /** AES 密钥字节数，用于校验 Base64 解码后的密钥长度。 */
    ...
    /** 密码学安全随机源，用于生成 AES 密钥和每次加密所需的唯一随机 IV。 */
     ...
     * 生成 256 位安全随机 AES 密钥。
     *
     * @return 标准 Base64 编码的 AES 密钥
     * @throws IllegalStateException 当前运行环境不支持 256 位 AES 时抛出
     */
    public static String generateKey() {
        try {
            ...
            throw new IllegalStateException("当前运行环境无法生成 AES 密钥", exception);
        }
    }
     ...
     * @param keyBase64 Base64 编码的 256 位 AES 密钥
     * @return Base64 编码的版本化 AES-GCM 密文
    ...
    public static String encrypt(String plaintext, String keyBase64) {
        ...
        try {
            ...
            throw CryptoSupport.invalidInput("AES-GCM 加密失败", exception);
        }
    }

    /**
     * 解密版本化 AES-GCM Base64 密文并验证认证标签。
     *
     * @param ciphertextBase64 Base64 编码的版本化 AES-GCM 密文
     * @param keyBase64 Base64 编码的 256 位 AES 密钥
     * @return UTF-8 明文
     * @throws IllegalArgumentException 参数、密钥、格式或认证标签不合法时抛出
     */
    public static String decrypt(String ciphertextBase64, String keyBase64) {
        byte[] envelope = CryptoSupport.decodeBase64(ciphertextBase64, "AES 密文");
        if (envelope.length < 1 + IV_SIZE_BYTES + TAG_SIZE_BYTES) {
            throw new IllegalArgumentException("AES 密文长度不合法");
        }
        if (envelope[0] != FORMAT_VERSION) {
            throw new IllegalArgumentException("不支持的 AES 密文格式版本");
        }
        SecretKey key = decodeKey(keyBase64);
        ...
        try {
            ...
            throw CryptoSupport.invalidInput("AES-GCM 解密失败，请检查密钥和密文", exception);
        }
    }

    private static SecretKey decodeKey(String keyBase64) {
        byte[] keyBytes = CryptoSupport.decodeBase64(keyBase64, "AES 密钥");
        if (keyBytes.length != KEY_SIZE_BYTES) {
            throw new IllegalArgumentException("AES 密钥必须为 256 位");
        }
