...
- 所有 Java 类、接口、枚举、记录和注解类型都必须在声明上方添加 JavaDoc，准确说明类型职责、适用边界和主要协作对象，不能只复述类型名称。
- 所有 Java 方法都必须添加 JavaDoc，包括 `public`、`protected`、包级、`private` 方法以及重写方法；不能用 `@Override` 代替用途说明。
- JavaDoc 应直接说明方法的作用或业务功能；存在参数、返回值或业务异常时，补充准确的 `@param`、`@return`、`@throws`，避免“处理数据”一类无信息量描述。
- 复杂实现需要在事务边界、状态转换、并发控制、补偿/回滚、算法关键步骤和不直观分支处添加行内注释，重点解释设计原因、前置条件或不变量，不要逐行翻译代码。
- 修改缺少类型或方法说明的既有 Java 代码时，应在同一次变更中补齐相关 JavaDoc；新增测试类、测试方法和测试辅助方法同样适用。
