# netty-rpc-protocol Specification

## Purpose

为基于 Netty 的自研 RPC 框架定义线上传输的自定义二进制协议：规定协议帧结构与编解码规则、基于长度字段解决 TCP 粘包/半包问题，并提供可插拔的序列化接口（默认基于 Jackson 实现 JSON 序列化），使客户端与服务端之间的消息交换具有统一、可扩展的格式。
