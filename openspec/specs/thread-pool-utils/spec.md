...
`ThreadPoolUtils` 提供统一封装的固定规格业务线程池，供需要异步执行任务的业务代码复用，替代
各处手写 `new ThreadPoolExecutor(...)` 或裸用 `Executors`：固定 4 线程、2048 容量有界队列、
可识别的线程命名、拒绝任务时记录日志再抛异常（不静默丢弃）、自动携带提交时刻的 traceId、
应用关闭时优雅终止。
